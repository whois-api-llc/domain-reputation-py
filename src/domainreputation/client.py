...
class Client:
    __default_url = "https://domain-reputation.whoisxmlapi.com/api/v1"
    ...
    def last_result(self, value: Response or None):
        if value is None:
            ...
            raise ValueError(
                "Values should be an instance of domainreputation.Response or None")
    ...
    def get(self, domain: str, mode: str = MODE_FAST) -> Response:
        """
        Get parsed API response as a `Response` instance.

        :key domain: Required. The website's domain name.
        :key mode: Optional. String.
            'fast' (Default) - some heavy tests and data collectors will be disabled.
            'full' - all the data and the tests will be processed. It costs 3 credits.
            Use Client.MODE_FAST and Client.MODE_FULL constants
        :return: `Response` instance
        :raises ConnectionError:
        :raises DomainReputationApiError: Base class for all errors below
        ...
        response = self.get_raw(domain, mode, Client._PARSABLE_FORMAT)
        try:
    ...
    def get_raw(self, domain: str, mode: str = MODE_FAST, output_format: str = _PARSABLE_FORMAT) -> str:
        """
        ...
        _mode = Client._validate_mode(mode)
    ...
    def _validate_mode(value: str) -> str:
        if value.lower() in [Client.MODE_FAST, Client.MODE_FULL]:
            return value.lower()

        raise ParameterError(
            f"Mode must be {Client.MODE_FAST} "
            f"or {Client.MODE_FULL}")
